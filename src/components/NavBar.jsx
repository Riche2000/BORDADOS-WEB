  ...
  return (
    <div className='flex justify-between items-center w-full h-20 text-stone-50 fixed bg-red-400 px-4'>
        ...
        <ul className='hidden md:flex'>
            {links.map(({id, link}) => (
                 <li key={id} className='px-4 cursor-pointer capitalize font-medium text-stone-50 hover:scale-105 duration-200'>{link}</li>
            ))}
        </ul>
        ...
        {nav &&(
        <ul className='flex flex-col justify-center items-center absolute top-0 left-0 w-full h-screen bg-gradient-to-b from-gray-800 to-stone-400 text-white'>        
        {links.map(({id, link}) => (
